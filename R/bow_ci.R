...
bow_ci <- function(bow_analysis_output, bootstraps = 1000, alpha = 0.95, window = 10, per_occurrence = TRUE, bootstrap_terms = TRUE){
  ...
  each_text <- function(x){
    ...
    each_phenomenon <- function(x){
      ...
      each_descriptor <- function(x){
        ...
        if(all(c(!is.nan(c(descriptor_i,mean_obs_i)),!anyNA(c(descriptor_i, mean_obs_i)))) == TRUE){
          ...
          if(bootstrap_terms == TRUE){
            ...
            bootstrap_occurrence <- function(input_vector){
              sample_index <- dqrng::dqsample(c(1:length(input_vector)), size = length(input_vector), replace = TRUE)
              sample <- (sum(input_vector[sample_index], na.rm = TRUE))/divisor
              return(sample)
            }
            #execute bootstrap
            bstrap_out <- furrr::future_map_dbl(1:bootstraps, ~bootstrap_occurrence(input_vector = sampling_vec))
            ...
            #execute boostrap
            bstrap_out <- furrr::future_map_dbl(1:bootstraps, ~boostrap_occurrence_sum(input_vector =  descriptor_i))
          }
      ...
      each_descriptor_out <- purrr::map_dfr(1:length(phenomenon_i[[2]]), ~each_descriptor(.x))
      return(each_descriptor_out)
    }
    #execute loop over phenomena
    each_phenomenon_out <- purrr::map_dfr(1:length(text_i), ~each_phenomenon(.x))
    each_phenomenon_out$phenomenon <- rep(names(text_i), each = (nrow(each_phenomenon_out)/length(text_i)))
    return(each_phenomenon_out)
  }
  #execute loop over texts
  if(length(bow_analysis_output) > 1){
    each_text_out <- furrr::future_map(1:length(bow_analysis_output), ~each_text(.x), .progress = TRUE)
  } else {
    each_text_out <- purrr::map(1:length(bow_analysis_output), ~each_text(.x))
  }
